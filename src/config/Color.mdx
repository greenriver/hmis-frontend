...
The swatches below are programmatically pulled from our theme, so they reflect the current state of the code,
not the desired state of the theme discipline. See Figma for desired changes.

From the advanced example here: https://storybook.js.org/docs/6/writing-docs/doc-blocks/doc-block-colorpalette
...
export function extractColors(paletteObject, prefix = '') {
  // The complexity/recursion here is because of the way we've setup Alerts colors.
